      ...
      <header className="topbar">
        <div className="brand-lockup">
          <div className="brand-logo-crop">
            <img
              src={`${import.meta.env.BASE_URL}images/sbi-official-lockup.png`}
              alt="State Bank of India"
            />
          </div>
          <div>
            <small>Centralised Monitoring System</small>
          </div>
        </div>
        <div className="top-signals" aria-label="System status">
          <span>
            <Radio size={14} /> 1,198 / 1,200 cameras online
          </span>
          <span className="alert-signal">
            <BellRing size={14} /> {critical} critical
          </span>
          <span>
            <Activity size={14} /> 99.84% availability
          </span>
        </div>
      ...
      <aside className={`sidebar ${compact ? "compact" : ""}`}>
        ...
        <div className="operator">
          ...
          {!compact && (
            <div>
              ...
              <small>LHO Supervisor • Mumbai</small>
            </div>
          )}
        </div>
        <nav>
        ...
        {!compact && (
          <div className="shift-card">
            <span>SHIFT 02</span>
            <strong>{live} active incidents</strong>
            <small>14:00–22:00 IST</small>
          </div>
        )}
      </aside>
      <main className={compact ? "compact-main" : ""}>
        <Outlet />
      </main>
      <footer className="statusbar">
        <span>
          <i /> All integrations nominal
        </span>
        <span>
          Video intelligence • Operator tools • Multilingual voice • Realtime data
        </span>
        <span>04 SEP 2026 • LIVE DEMO</span>
      </footer>
    </div>
  );
}
