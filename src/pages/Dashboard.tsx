    ...
    <div className="grid-main">
      <div className="stack">
        <section className="panel"><div className="panel-head"><h2>Priority incident queue</h2><span>REALTIME • REPRESENTATIVE PILOT DATA</span></div><div className="incident-list">
          {incidents.slice(0,5).map(i=><Link className="incident-row" to={`/incidents/${i.id}`} key={i.id}><span className={`badge ${i.severity}`}>{i.severity}</span><div><strong>{i.title}</strong><small>{i.id} • {i.useCase}</small></div><div><strong>{i.branch}</strong><small>{i.camera}</small></div><div><strong>{i.ageMinutes} min</strong><small>SLA {i.slaMinutes} min</small></div><span className={`badge ${i.status}`}>{i.status.replace('_',' ')}</span></Link>)}
        </div></section>
